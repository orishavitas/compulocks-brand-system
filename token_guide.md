# Compulocks Brand Token Guide
> Auto-generated from tokens/*.json — do not edit directly

## Colors

| Token | Value | Description |
|-------|-------|-------------|
| `color.brand.primary` | `#1D1F4A` | Main brand color — Compulocks navy blue |
| `color.brand.highlight` | `#243469` | Secondary brand highlight |

## Typography

### Font Families & Weights

| Token | Value | Description |
|-------|-------|-------------|
| `font.family.primary` | `'Barlow Condensed', sans-serif` | Primary brand font — headlines and big text |
| `font.family.secondary` | `'Barlow', sans-serif` | Secondary brand font — body text |
| `font.weight.regular` | `400` | Regular weight — body text and captions |
| `font.weight.medium` | `500` | Medium weight — headlines and emphasis |

### Text Styles

| Style | Font | Weight | Transform | Description |
|-------|------|--------|-----------|-------------|
| `textStyle.bigShortTitle` | {font.family.primary} | {font.weight.medium} | uppercase | Large short headlines — all caps |
| `textStyle.bigLongTitle` | {font.family.primary} | {font.weight.medium} | capitalize | Large long headlines — title case |
| `textStyle.bigParagraph` | {font.family.primary} | {font.weight.regular} | lowercase | Large body text |
| `textStyle.smallParagraph` | {font.family.secondary} | {font.weight.regular} | lowercase | Small body text |
| `textStyle.smallText` | {font.family.secondary} | {font.weight.regular} | lowercase | Small italic text — captions, notes |

## Spacing Scale

| Token | Value | px | Description |
|-------|-------|-----|-------------|
| `spacing.1` | `0.25rem` | 4px |
| `spacing.2` | `0.5rem` | 8px |
| `spacing.3` | `0.75rem` | 12px |
| `spacing.4` | `1rem` | 16px |
| `spacing.5` | `1.25rem` | 20px |
| `spacing.6` | `1.5rem` | 24px |
| `spacing.8` | `2rem` | 32px |
| `spacing.10` | `2.5rem` | 40px |
| `spacing.12` | `3rem` | 48px |
| `spacing.16` | `4rem` | 64px |
